...
function switchAdminTab(tabName) {
    ...
    const tabMap = { orders: 0, logs: 1, products: 2, stats: 3, users: 4, keys: 5 };
    document.querySelectorAll('.admin-tab')[tabMap[tabName]].classList.add('active');
...
async function renderKeys() {
    ...
    if (keys.length === 0) {
        keysList.innerHTML = '<p style="color:rgba(255,255,255,0.4);text-align:center;padding:30px;">Brak wydanych kluczy. Złóż zamówienie, aby zobaczyć tu klucz.</p>';
        return;
    }
...
async function deleteKey(orderId) {
    showConfirmModal('Usunąć klucz?', 'Czy na pewno chcesz usunąć ten klucz? Tej operacji nie można cofnąć.', async function() {
...
async function renderKeys() {
    ...
    const keys = await loadKeys();
    const searchQuery = document.getElementById('keys-search-input')?.value.trim().toLowerCase() || '';
    
    const filteredKeys = keys.filter(k => 
        k.key.toLowerCase().includes(searchQuery) || 
        k.customer.toLowerCase().includes(searchQuery) ||
        k.email.toLowerCase().includes(searchQuery)
    );
    
    keysList.innerHTML = '';
    
    if (filteredKeys.length === 0) {
        keysList.innerHTML = '<p style="color:rgba(255,255,255,0.4);text-align:center;padding:30px;">Brak kluczy</p>';
        return;
    }
    ...
    filteredKeys.forEach(keyData => {
        ...
        const statusClass = keyData.status === 'Dostarczone' || keyData.status === 'Opłacone' ? 'active' : 'used';
        const statusLabel = keyData.status === 'Dostarczone' ? 'Wydany' : keyData.status;
        
        card.innerHTML = `
            <div class="key-info">
                <div class="key-value">${keyData.key}</div>
                <div class="key-customer">${keyData.customer} (${keyData.email})</div>
                <div class="key-customer">${keyData.date}</div>
            </div>
            <span class="key-status ${statusClass}">${statusLabel}</span>
            <div class="key-actions">
                <button class="copy-btn" onclick="copyKey('${keyData.key}')">Kopiuj</button>
                <button class="delete-key-btn" onclick="deleteKey('${keyData.orderId}')">Usuń</button>
            </div>
