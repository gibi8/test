...
function hidePreloader() {
    ...
    // Natychmiast ustaw na 100%
    updateProgress(100);
    
    // Poczekaj 400ms na wypełnienie, potem ukryj
    setTimeout(() => {
...
function startProgress() {
    ...
    // Czyścimy stary interval jeśli istnieje
    if (progressInterval) {
        clearInterval(progressInterval);
        progressInterval = null;
    }
    ...
    progressInterval = setInterval(() => {
        ...
        // Symulacja ładowania - najpierw szybciej, potem wolniej
        if (step < 10) {
            progress += 2.5 + Math.random() * 1.5;
        } else if (step < 20) {
            progress += 1.8 + Math.random() * 1.2;
        } else if (step < 35) {
            progress += 1.2 + Math.random() * 1.0;
        } else {
            progress += 0.5 + Math.random() * 0.8;
        }
        
        // Nie przekraczaj 95%
        if (progress >= 95) {
            progress = 95;
            // Wyczyść interval po osiągnięciu 95%
            if (progressInterval) {
                clearInterval(progressInterval);
                progressInterval = null;
            }
...
// Dodajemy małe opóźnienie startowe dla lepszego wrażenia
setTimeout(() => {
    // Kontynuuj ładowanie
}, 300);
...
window.addEventListener('DOMContentLoaded', async function() {
    try {
        // Minimalny czas wyświetlania preloadera (2.5 sekundy)
        ...
        const minDisplayTime = 2800; // minimum 2.8 sekundy
        
        // Jeśli załadowało się szybciej niż minDisplayTime, poczekaj
...
// Bezpiecznik - ukryj preloader po max 6 sekundach
setTimeout(function() {
    hidePreloader();
}, 6000);

console.log('✅ NEXUS MARKET - Załadowano pomyślnie!');
...
async function updateTicketsCount() {
    // Funkcja pusta - tickety są aktualizowane przez real-time
}
        ...
        if (ticket.status === 'Zamknięty') {
            ...
            if (replySection) {
                replySection.innerHTML = `
                    ...
                    <button class="submit-btn" onclick="sendAdminReplyRealTime('${ticket.id}')">Wyślij odpowiedź</button>
                `;
            }
        }
    });
...
function initStars() {
    try {
        ...
        if (!canvas) {
            console.log('Canvas nie znaleziony');
            return;
        }
...
function resetFilters() {
    ...
    renderFilteredProducts(products);
    document.getElementById('products-count').textContent = products.length + ' produktów';
}
...
function renderFilteredProducts(filteredProducts) {
    ...
    if (filteredProducts.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:rgba(255,255,255,0.4);grid-column:1/-1;padding:40px;">Brak produktów spełniających kryteria</p>';
        return;
    }
    ...
    filteredProducts.forEach(product => {
        ...
        const imageUrl = product.image && product.image.trim() !== '' 
            ? product.image 
            : 'https://via.placeholder.com/300x300/0c121c/3b82f6?text=N';
        
        const price = product.price || 0;
        const priceDisplay = price.toFixed(2) + ' zł';
        
        card.innerHTML = `
            <div class="product-image" onclick="event.stopPropagation(); openProductPage('${product.id}')">
                <img src="${imageUrl}" alt="${product.name}" 
                     onerror="this.src='https://via.placeholder.com/300x300/0c121c/3b82f6?text=N'">
            </div>
            <div class="product-info">
                <div class="product-name" onclick="event.stopPropagation(); openProductPage('${product.id}')">${product.name}</div>
                <div class="product-price" onclick="event.stopPropagation(); openProductPage('${product.id}')">${priceDisplay}</div>
                <hr class="product-divider">
                <div class="product-footer">
                    <span class="product-stock">${stock} ${stock === 1 ? 'szt.' : 'szt.'}</span>
                    <div class="product-actions">
                        <button class="btn-cart" onclick="event.stopPropagation(); addToCart('${product.id}')">Dodaj</button>
                        <button class="btn-buy" onclick="event.stopPropagation(); buyNowFromProduct('${product.id}')">Kup</button>
                    </div>
                </div>
            </div>
        `;
...
function buyNowFromProduct(productId) {
    addToCart(productId);
    checkout();
}
...
function openProductPage(productId) {
    ...
    const imageUrl = product.image && product.image.trim() !== '' 
        ? product.image 
        : 'https://via.placeholder.com/400x300/0c121c/3b82f6?text=N';
    
    if (img) {
        img.src = imageUrl;
        img.onerror = function() {
            this.src = 'https://via.placeholder.com/400x300/0c121c/3b82f6?text=N';
        };
    }
...
async function renderReviews() {
    try {
        ...
        const allReviewsCombined = [...reviews, ...defaultReviews];
        
        allReviews = allReviewsCombined.map(r => {
...
// ============ SORTOWANIE OPINII ============
function sortReviews(sortType, button) {
...
function renderReviewCards(reviews) {
    ...
    reviews.forEach(review => {
        ...
        if (review.createdAt) {
            ...
            if (!isNaN(date)) {
                dateStr = date.toLocaleDateString('pl-PL', { month: 'short', day: 'numeric', year: 'numeric' });
            }
        }
...
async function submitReview() {
    ...
    if (currentOrderData && currentOrderData.items && currentOrderData.items.length > 0) {
        productName = currentOrderData.items[0].name || 'Produkt';
    }
...
function addToCart(productId) {
    ...
    const cartEl = document.querySelector('.nm-cart');
    if (cartEl) {
...
function updateCartUI() {
        ...
        if (cart.length === 0) {
            cartItems.innerHTML = `
                <div class="cart-empty">
                    ...
                    <button class="browse-btn" onclick="closeCart(); showPage('products')">Przeglądaj produkty</button>
                </div>
            `;
        } else {
            cart.forEach(item => {
                ...
                const imageUrl = item.image && item.image.trim() !== '' 
                    ? item.image 
                    : 'https://via.placeholder.com/56/0c121c/3b82f6?text=N';
                div.innerHTML = `
                    ...
                    <div class="cart-item-controls">
                        <div class="cart-item-quantity">
                            <button class="qty-btn" onclick="changeQuantity('${item.id}', -1); updateCartUI();">−</button>
                            <span>${item.quantity}</span>
                            <button class="qty-btn" onclick="changeQuantity('${item.id}', 1); updateCartUI();">+</button>
                        </div>
                        <button class="remove-item-btn" onclick="removeFromCart('${item.id}'); updateCartUI();">Usuń</button>
                    </div>
                `;
    ...
    // Aktualizacja strony koszyka jeśli jest otwarta
    ...
    if (saved) {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                // Sprawdź czy produkty istnieją w bazie
...
function clearCart() {
    ...
    showConfirmModal(
        'Opróżnić koszyk?', 
        'Czy na pewno chcesz usunąć wszystkie produkty z koszyka? Tej operacji nie można cofnąć.',
        function() {
            cart = [];
            saveCartToStorage();
            updateCartUI();
            showNotification('Koszyk został opróżniony', 'error');
            closeCart();
        }
    );
}
...
function renderCartPage() {
    ...
    if (cart.length === 0) {
        container.innerHTML = `
            <div class="cart-page-empty">
                ...
                <h3>Twój koszyk jest pusty</h3>
                <p>Dodaj produkty, aby rozpocząć zakupy.</p>
                <button class="browse-btn" onclick="showPage('products')">Przeglądaj produkty</button>
            </div>
        `;
    ...
    cart.forEach(item => {
        ...
        const imageUrl = item.image && item.image.trim() !== '' 
            ? item.image 
            : 'https://via.placeholder.com/70/0c121c/3b82f6?text=N';
            ...
            <div class="cart-page-item" data-id="${item.id}">
                ...
                <div class="cart-page-item-controls">
                    <div class="cart-page-item-quantity">
                        <button class="cart-page-qty-btn" onclick="changeQuantity('${item.id}', -1); renderCartPage();">−</button>
                        <span>${item.quantity}</span>
                        <button class="cart-page-qty-btn" onclick="changeQuantity('${item.id}', 1); renderCartPage();">+</button>
                    </div>
                    <button class="cart-page-remove-btn" onclick="removeFromCart('${item.id}'); renderCartPage();">Usuń</button>
                </div>
            </div>
        `;
    });
...
async function renderAdminProducts() {
    ...
    products.forEach(product => {
        ...
        const imageUrl = product.image && product.image.trim() !== '' 
            ? product.image 
            : 'https://via.placeholder.com/50/0c121c/3b82f6?text=N';
...
document.getElementById('edit-product-form').addEventListener('submit', async function(e) {
    ...
    const description = document.getElementById('edit-description').value.trim() || 'Brak opisu.';
...
async function addProduct() {
    ...
    const description = document.getElementById('admin-description').value.trim() || 'Brak opisu.';
    const featuresInput = document.getElementById('admin-features').value.trim();
    const specsInput = document.getElementById('admin-specs').value.trim();
...
// ============ EVENT LOAD - JEDEN, POPRAWNY ============
window.addEventListener('DOMContentLoaded', async function() {
    try {
        // Symulacja ładowania - progress bar (wolniejsze)
        let progress = 0;
        ...
        // Małe opóźnienie dla lepszego wrażenia
...
// Bezpiecznik - ukryj preloader po max 5 sekundach
setTimeout(function() {
    hidePreloader();
}, 5000);
